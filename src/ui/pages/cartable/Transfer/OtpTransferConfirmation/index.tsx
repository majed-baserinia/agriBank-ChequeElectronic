		...
		if (otpTransferRequirments && otpTransferRequirments.transferChequeKey) {
			...
			if (transferAction === 'confirm') {
				verifyOtpTransfer(requestBody, {
					...
					onSuccess: (res) => {
						addNewCartableData({ transferOverview: res });
						navigate(paths.cartable.TransferOverView);
						//TODO:
						//if ('hoghooghi') navigate(  paths.cartable.TransferSignatureGroup);
						//if ('haghighi') navigate(  paths.cartable.Detail);
					}
				});
			}
			if (transferAction === 'reject') {
				verifyOtpReject(requestBody, {
					...
					onSuccess: (res) => {
						addNewCartableData({ transferOverview: res as TransferChequeVerifyOtpResponse });
						navigate(paths.cartable.TransferOverView);
						//TODO:
						//if ('hoghooghi') navigate(  paths.cartable.TransferSignatureGroup);
						//if ('haghighi') navigate(  paths.cartable.Detail);
