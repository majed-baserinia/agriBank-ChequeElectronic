...
import HandlerInstance from '@Mediatr/models/handler.instance.js';
...
export default class Resolver implements IResolver {
	 ...
	 * @memberof Resolver
	 */
	public resolveMany<T>(name: string): T[] {
		const e = this._instances.filter((p) => p.name === name);

		// eslint-disable-next-line @typescript-eslint/no-explicit-any

		return e.map((x) => new (x.value as any)());
	}
