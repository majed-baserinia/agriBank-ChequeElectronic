...
import { Constrain } from '@Fluentvalidator/types/Constrain';
import { ArrayValueValidatorBuilder } from '@Fluentvalidator/valueValidator/ArrayValueValidatorBuilder';
import { AsyncArrayValueValidatorBuilder } from '@Fluentvalidator/valueValidator/AsyncArrayValueValidatorBuilder';
import { AsyncValueValidatorBuilder } from '@Fluentvalidator/valueValidator/AsyncValueValidatorBuilder';
import { AsyncRuleValidators, RuleValidators } from '@Fluentvalidator/valueValidator/RuleValidators';
import { ValueTransformer } from '@Fluentvalidator/valueValidator/ValueTransformer';
import { hasError } from '@Fluentvalidator/valueValidator/ValueValidator';
import { ValueValidatorBuilder } from '@Fluentvalidator/valueValidator/ValueValidatorBuilder';
		...
		TModel[propertyName] extends Array<infer TEachValue>
			?
					| ValueValidatorBuilder<
							TModel,
							TModel[propertyName],
							TModel[propertyName] | string | number | boolean | null | undefined | symbol
					  >
...
export abstract class CoreValidator<TModel, TAsync extends true | false> {
	...
	protected _validateAsync: (value: TModel) => Promise<ValidationErrors<TModel>> = async () => {
	...
	private rebuildValidate = () => {
		this._validate = (value: TModel): ValidationErrors<TModel> => {
			...
			for (const propertyName of Object.keys(this.valueValidatorBuildersByPropertyName)) {
				const valueValidatorBuilders = this.valueValidatorBuildersByPropertyName[propertyName as keyof TModel];
	...
	private rebuildValidateAsync = () => {
		this._validateAsync = async (value: TModel): Promise<ValidationErrors<TModel>> => {
			const errors: ValidationErrors<TModel> = {};

			for (const propertyName of Object.keys(this.asyncValueValidatorBuildersByPropertyName)) {
				const asyncValueValidatorBuilders =
					this.asyncValueValidatorBuildersByPropertyName[propertyName as keyof TModel];
	...
	): TAsync extends true ? AsyncRuleValidators<TModel, TValue> : RuleValidators<TModel, TValue> => {
		if (this._isAsync) {
			const asyncValueValidatorBuilder = new AsyncValueValidatorBuilder<TModel, TValue, TValue>(
			...
			this.asyncValueValidatorBuildersByPropertyName[propertyName]!.push(asyncValueValidatorBuilder as any);

			return asyncValueValidatorBuilder.getAllRules() as unknown as AsyncRuleValidators<TModel, TValue> as any; // Appease the type system
		} else {
			...
			this.valueValidatorBuildersByPropertyName[propertyName]!.push(valueValidatorBuilder as any);

			return valueValidatorBuilder.getAllRules() as unknown as RuleValidators<TModel, TValue> as any; // Appease the type system
		}
	};
	...
	>(
		...
		transformValue: (
			value: TValue
		) => TTransformedValue extends object ? Constrain<TTransformedValue, TValue> : TTransformedValue
	): TAsync extends true
		? AsyncRuleValidators<TModel, TTransformedValue>
		: RuleValidators<TModel, TTransformedValue> => {
		if (this._isAsync) {
			const asyncValueValidatorBuilder = new AsyncValueValidatorBuilder<TModel, TValue, TTransformedValue>(
				this.rebuildValidateAsync,
			...
			return valueValidatorBuilder.getAllRules() as unknown as RuleValidators<TModel, TTransformedValue> as any; // Appease the type system
		}
	};
	...
	protected ruleForEach = <
		...
		TValue extends TModel[TPropertyName] extends
			...
			| null
			| undefined
			? TModel[TPropertyName] & (Array<TEachValue> | ReadonlyArray<TEachValue> | null | undefined)
			: never
	>(
		propertyName: TModel[TPropertyName] extends Array<unknown> | ReadonlyArray<unknown> | null | undefined
			? TPropertyName
			: never
	): TValue extends Array<unknown> | ReadonlyArray<unknown>
		? TAsync extends true
		...
		if (this._isAsync) {
			...
			this.asyncValueValidatorBuildersByPropertyName[propertyName]!.push(asyncArrayValueValidatorBuilder as any);
			...
			this.valueValidatorBuildersByPropertyName[propertyName]!.push(arrayValueValidatorBuilder as any);
	...
	>(
		propertyName: TModel[TPropertyName] extends Array<unknown> | ReadonlyArray<unknown> | null | undefined
			...
			: never,
		transformValue: (
			value: TModel[TPropertyName] extends Array<infer TEachValue> | ReadonlyArray<infer TEachValue>
				? TEachValue
				: never
		) => TEachTransformedValue extends object
			? Constrain<
					TEachTransformedValue,
					TModel[TPropertyName] extends Array<infer TEachValue> | ReadonlyArray<infer TEachValue>
						? TEachValue
						: never
				>
			: TEachTransformedValue
