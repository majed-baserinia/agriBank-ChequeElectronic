...
describe('number validators', () => {
	describe('lessThan', () => {
		...
		it('throws an error if it receives a non-number value', () => {
			...
			expect(() => otherValidator.validate({ name: 'Alex' })).toThrowError(TypeError);
		});
	...
	describe('greaterThanOrEqualTo', () => {
		...
		it('gives a validation error if the value is less than the max value', () => {
			...
			const result = validator.validate(invalid);
			expect(result.nullableNumberProperty).toBe(`Value must be greater than or equal to ${minValue}`);
		});
	...
	describe('scalePrecision', () => {
		...
		it('handles large scales and precisions', () => {
			...
			const validValues = [
				1234567.1234567, 0.9999999, 9999999, 9999999.9999999, -1234567.1234567, -0.9999999, -9999999,
				-9999999.9999999
			];
