...
describe('string validators (sync)', () => {
	describe('notEmpty', () => {
		...
		it('throws an error if it receives a non-string value', () => {
			...
			expect(() => otherValidator.validate({ age: 10 })).toThrowError(TypeError);
		});
	...
	describe('maxLength', () => {
		...
		it('gives a validation error if the value is too long', () => {
			...
			const result = validator.validate(invalid);
			expect(result.nullableStringProperty).toBe(`Value must be no more than ${maxLength} characters long`);
		});
	...
	describe('minLength', () => {
		...
		it('gives a validation error if the value is too short', () => {
			...
			const result = validator.validate(invalid);
			expect(result.nullableStringProperty).toBe(`Value must be at least ${minLength} characters long`);
		});
