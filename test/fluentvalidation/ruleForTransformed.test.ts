	...
	describe('sync', () => {
		...
		describe('array type', () => {
			...
			it('can be transformed to an array of the same type', () => {
				const addFooToEachElement = (value: Array<string>): Array<string> => value.map((x) => 'foo' + x);
			...
			it('cannot be transformed to an object of a different type', () => {
				...
				class TestValidator extends AsyncValidator<TestType> {
					constructor() {
						...
						// @ts-expect-error
						this.ruleForTransformed('objectProperty', transform).must((x) => x.newProperty === 'whatever');
					}
				}
			});
