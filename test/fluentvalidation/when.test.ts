import { AsyncValidator, Validator } from '@Fluentvalidator/index';
...
describe('when', () => {
	describe('sync', () => {
		describe('if applied to all validators', () => {
			class TestValidator extends Validator<TestType> {
				constructor() {
					...
					this.ruleFor('nullableStringProperty')
						.notNull()
						.notEmpty()
						.when((model) => model.booleanProperty, 'AppliesToAllValidators');
				}
			}
			const validator = new TestValidator();

			it('gives a validation error if any rule fails and the condition is met', () => {
				const invalid = {
					...testInstance,
			...
			it('does not give a validation error if the value is invalid and the condition is not met', () => {
				const invalid = {
					...testInstance,
		...
		describe('if applied to only the current validator', () => {
			...
			it('gives a validation error if the latest rule fails and the condition is met', () => {
				const invalid = {
					...testInstance,
					booleanProperty: true,
			...
			it('does not give a validation error if the latest rule fails and the condition is not met', () => {
				const invalid = {
					...testInstance,
					booleanProperty: false,
					nullableStringProperty: ' '
	...
	describe('async', () => {
		describe('if applied to all validators', () => {
			...
			it('gives a validation error if any rule fails and the condition is met', async () => {
				const invalid = {
					...testInstance,
					booleanProperty: true,
					nullableStringProperty: null
			...
			it('does not give a validation error if the value is invalid and the condition is not met', async () => {
				const invalid = {
					...testInstance,
					booleanProperty: false,
					nullableStringProperty: null
		...
		describe('if applied to only the current validator', () => {
			...
			it('gives a validation error if the latest rule fails and the condition is met', async () => {
				const invalid = {
					...testInstance,
					booleanProperty: true,
					nullableStringProperty: '  '
			...
			it('does not give a validation error if the latest rule fails and the condition is not met', async () => {
				const invalid = {
					...testInstance,
					booleanProperty: false,
					nullableStringProperty: ' '
